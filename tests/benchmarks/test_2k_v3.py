"""Test v5_parallel_v3 with 2k terms."""
...
if __name__ == "__main__":
    print("2k term v3 test\n")

    # Check for cached 2k subset
    cache_path = os.path.join(os.path.dirname(__file__), 'cached_2k_subset.pkl')
    if os.path.exists(cache_path):
        print("Loading cached 2k subset...")
        import pickle
        with open(cache_path, 'rb') as f:
            P_small, nqubits = pickle.load(f)
        print(f"  Loaded {len(P_small.terms)} terms")
    else:
        print("Creating 2k subset from scratch (will cache for next time)...")
