"""Test v5_parallel_v3 with 20k terms."""
...
if __name__ == "__main__":
    print("20k term v3 test\n")

    # Check for cached 20k subset
    cache_path = os.path.join(os.path.dirname(__file__), 'cached_20k_subset.pkl')
    if os.path.exists(cache_path):
        print("Loading cached 20k subset...")
        import pickle
        with open(cache_path, 'rb') as f:
            P_small, nqubits = pickle.load(f)
        print(f"  Loaded {len(P_small.terms)} terms")
    else:
        print("Creating 20k subset from scratch (will cache for next time)...")
        ...
        P_small = create_random_subset(hamiltonian_full, 20000, seed=44)
        # Cache for next time
        import pickle
    ...
    # v5_parallel_v3 with 3 workers
    print("\nRunning v5_parallel_v3 (3 workers)...")
    start = time.time()
    groups_v3 = get_si_sets_v5_parallel_v3(terms_ord, blocks_kN, verbosity=1, num_workers=3)
