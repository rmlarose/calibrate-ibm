...
def get_lucj_ansatz(alpha_alpha_indices: list[tuple[int, int]],
                    alpha_beta_indices: list[tuple[int, int]],
                    spin_a_layout: list[int],
                    spin_b_layout: list[int],
                    num_orbitals: int,
                    num_electrons: tuple[int,int],
                    n_reps: int,
                    use_ccsd_params: bool,
    ...
    """
    alpha_alpha_indices: List of tuples indicating the indices of alpha-alpha interaction pairs for the LUCJ ansatz.
    alpha_beta_indices: List of tuples indicating the indices of alpha-beta interaction pairs for the LUCJ ansatz.
    spin_a_layout: List of qubit indices for spin-up orbitals in the quantum circuit.
    spin_b_layout: List of qubit indices for spin-down orbitals in the quantum circuit.
    num_orbitals: Total number of spatial orbitals in the system.
    num_electrons: Total number of electrons in the system.
    n_reps: Number of repetitions of the LUCJ ansatz layers.
    use_ccsd_params: Boolean flag indicating whether to use CCSD parameters for the ansatz.
    one_body_integrals: One-body integrals (h1) as a numpy array.
    two_body_integrals: Two-body integrals (h2) as a numpy array.
    backend: Qiskit backend for transpilation and optimization.
    """

    qubits = QuantumRegister(int(2 * num_orbitals), name="q")
    circuit = QuantumCircuit(qubits)
    if use_ccsd_params:

        mol_custom = gto.M()
        mol_custom.nelectron = num_electrons[0] + num_electrons[1]
        mol_custom.incore_anyway = True

        rhf_custom = scf.RHF(mol_custom)
        rhf_custom.get_hcore = lambda *args: one_body_integrals
        rhf_custom.get_ovlp = lambda *args: np.eye(num_orbitals)
        rhf_custom._eri = ao2mo.restore(1, two_body_integrals, num_orbitals)
        rhf_custom.init_guess = 'minao'
        rhf_custom.kernel()

        ccsd = cc.CCSD(mf=rhf_custom, mo_coeff=rhf_custom.mo_coeff)
        ccsd.kernel()
    ...
    circuit.append(ffsim.qiskit.PrepareHartreeFockJW(int(num_orbitals), (int(num_electrons[0]), int(num_electrons[1]))), qubits)

    # apply the UCJ operator to the reference state
    circuit.append(ffsim.qiskit.UCJOpSpinBalancedJW(ucj_op), qubits)
    circuit.measure_all()
    
    if spin_a_layout is None or spin_b_layout is None:
        initial_layout = None
    else:
